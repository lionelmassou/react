...
myString = JSON.stringify(this.myJSON)
  ...
  render() {
    ...
    return (
      <BrowserRouter> {/* on place ce composant autour de tout le reste de l'application */}
        <div>
          <nav> {/* ici, la nav apparaîtra sur toutes les pages, donc on ne l'appelle qu'une fois dans App */}
            <ul>
              <li><Link to="/Movie">Home</Link></li> {/* Chaque lien est un composant <Link> */}
            </ul>
          </nav>

          <Switch> {/* Ce composant ne s'affichera pas à l'écran, il liste les chemins possibles */}
            <Route exact path="/:id" component={Movie} /> {/* Chaque route est un composant <Route> */}
